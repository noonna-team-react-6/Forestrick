  ...
  return (
    <section className="more-edit-options">
      <div className="more-edit-header">
        <div>
          <h2>
            <IconWand size={18} />더 다양한 편집 옵션
          </h2>

          <p>원하는 편집 방향을 선택하거나 직접 요청해 보세요.</p>
        </div>
      ...
      <div className="edit-option-groups">
        {Object.entries(EDIT_OPTIONS).map(([category, options]) => (
          <div className="edit-option-group" key={category}>
            <span className="edit-option-category">{category}</span>
            ...
            <div className="edit-option-buttons">
              {options.map((option) => (
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  selected={selectedAction === option.label}
      ...
      <div className="custom-request">
        <span className="custom-request-icon">✎</span>
